...
public enum StringParser {
    ...
    public String[] parseStrings(int option, String delimiter, String... args) {
        ...
        // if OPTION is lower than -2, it is bad argument
        if (option < -2)
            throw new IllegalArgumentException("Option cannot be lower than -2");

        // if OPTION is -2, string parsing is disabled
        if (option == -2)
            return args;

        if (option == -1) {
            // default string identifying; processing " chars
            ...
            // concat items after index OPTION; not processing " chars
